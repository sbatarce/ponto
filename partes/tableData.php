...
if( $qry == "correcs" )
	...
	$sql	=	"SELECT FUCO_ID, FUNI_ID, FUAU_ID, 
									...
									FUCO_DCDBCR AS DBCR, FUCO_DLOBS AS OBS, 
									BIOMETRIA.SF_TOHHMM(FUCO_NITMP) AS TMP
...
if( $qry == "funcfuor" )
	...
	if( !isset( $_GET["dtfech"] ) )
		{
		echo	'{ "data": [{"erro": "parametro dtfech obrigatorio"}] }';
		return;
		}
	$uor = $_GET["uor"];
	$dtfech = $_GET["dtfech"];
	$sql = "SELECT	FUNI.PMS_IDPMSPESSOA AS SSHD,
                                ...
                                TO_CHAR( TO_DATE( '$dtfech', 'YYYYMMDD' ), 'DD/MM/YYYY' ) )
									...
									BIOMETRIA.SF_CALCULASALDOINICIAL( FUNI.FUNI_ID, TO_DATE('$dtfech','YYYYMMDD')+1 ) AS SALDO,
									0 AS FECHAR
...
if( $qry == "uorautos" )
	...
	if( isset( $_GET["uorid"] ) )
		...
		$sql = "SELECT	FUAU.FUAU_ID AS FUAUID, FUAU.FUNI_ID, FUNI.PMS_IDPMSPESSOA AS SSHD, 
										...
										VUOR.UOR_DLSIGLAUNIDADE AS SIGLAUOR
							FROM        BIOMETRIA.FUAU_FUNCIONARIOAUTORIZADOR FUAU
							INNER JOIN  BIOMETRIA.FUNI_FUNCIONARIO FUNI ON
													FUNI.FUNI_ID = FUAU.FUNI_ID
							INNER JOIN  SAU.VWUORPUBLICA VUOR ON 
													VUOR.UOR_IDUNIDADEORGANIZACIONAL = FUAU.PMS_IDSAUUOR
							WHERE FUAU.PMS_IDSAUUOR = $uorid
							ORDER BY	SIGLAUOR, FUAU.FUAU_DTFIM DESC, 
												FUAU.FUAU_DTINICIO, NOME";
		}
	else
		{
		$sql = "SELECT	FUAU.FUAU_ID AS FUAUID, FUAU.FUNI_ID, FUNI.PMS_IDPMSPESSOA AS SSHD, 
...
if( $qry == "funcapar" )
	...
	$sql	=	"SELECT	FUNI.FUNI_ID, FUNI.PMS_IDPMSPESSOA AS SSHD,
						...
						INNER JOIN  SAU.VWUSUARIO_SSHD PESS
									ON    PESS.CDUSUARIO=FUNI.PMS_IDPMSPESSOA
									AND   PESS.USUARIO_ATIVO=1
...
if( $qry == "autcadas" )
	...
	if( isset( $_GET["sshd"] ) )
		...
		$sql = "SELECT  SUOR.UOR_DLSIGLAUNIDADE AS UNIDADE, FUNI.FUNI_ID AS IDFUNI,
										FUNI.PMS_IDPMSPESSOA AS SSHD, FUNI.FUNI_STMENS AS STENVMENS,
										CASE
											WHEN FUNI.FUNI_STMENS = 1 THEN 'sim'
											ELSE 'nao'
											END AS ENVMENS,								
							...
							WHERE				SYSDATE BETWEEN 
														...
														NVL(FUAU.FUAU_DTFIM, TO_DATE('31/12/3000', 'DD/MM/YYYY'))+1
...
if( $qry == "ausaut" )
	...
	$sql = "select  TAAU.TAAU_ID AS IDTIPO, TAAU.TAAU_DLAUSENCIAAUTORIZADA as TIPO, 
									TAAU.TAAU_STMARCACAO as PODE, FAAU.FAAU_STLIB4BATIDAS AS LIB4B,
		...
		$sql	=	"SELECT	FUNI.FUNI_ID AS IDFUNC, FUNI.PMS_IDPMSPESSOA AS SSHDFUNC, 
							...
							INNER JOIN  BIOMETRIA.FUAU_FUNCIONARIOAUTORIZADOR FUAU ON 
													SYSDATE BETWEEN FUAU.FUAU_DTINICIO AND 
															NVL(FUAU.FUAU_DTFIM, TO_DATE('31/12/3000', 'DD/MM/YYYY'))+1 AND 
							...
							GROUP BY  FUNI.FUNI_ID, FUNI.PMS_IDPMSPESSOA, VUPU.UOR_DLSIGLAUNIDADE, 
												FUOR.PMS_IDSAUUOR, FSHM.FSHM_DTREFERENCIA 
		...
		$sql = "SELECT	FUNI.FUNI_ID AS IDFUNC, FUNI.PMS_IDPMSPESSOA AS SSHDFUNC, 
							...
							LEFT JOIN   BIOMETRIA.FDTR_FUNCIONARIODIATRABALHO FDTR ON 
													FDTR.FDTR_DTREFERENCIA BETWEEN 
																FSHM.FSHM_DTREFERENCIA AND SYSDATE AND 
													FDTR.FRTR_ID = FRTR.FRTR_ID 
							WHERE     FUNI.PMS_IDPMSPESSOA <> '$autsshd'
							GROUP BY  FUNI.FUNI_ID, FUNI.PMS_IDPMSPESSOA, VUPU.UOR_DLSIGLAUNIDADE, 
												FUOR.PMS_IDSAUUOR, FSHM.FSHM_DTREFERENCIA 
							ORDER BY  UNIDADE, QTPENDENTE DESC";
		}	
...
if( $qry == "pendencias" )
	...
	$sql	=	"SELECT		FUNI.FUNI_ID, FDTR.FDTR_ID, FDTR.TSDT_ID, FUNI.PMS_IDPMSPESSOA as SSHD, PESS.NOME,
										...
										(SELECT LISTAGG(TO_CHAR(FDTE.FDTE_DTHORARIO, 'DD/MM/YYYY HH24:MI'), ';') 
                             ...
                             WHERE FDTN.FDTR_ID = FDTR.FDTR_ID) AS MENSPADRAO,
										...
										(SELECT CASE FUCO_DCDBCR
															WHEN 'DB' THEN BIOMETRIA.SF_TOHHMM(FUCO_NITMP) || 'DB'
															WHEN 'CR' THEN BIOMETRIA.SF_TOHHMM(FUCO_NITMP) || 'CR' END
												FROM BIOMETRIA.FUCO_FUNCCORRECAOHORAS 
												WHERE FUCO_DTREFERENCIA=FDTR.FDTR_DTREFERENCIA
													AND FUNI_ID=FUNI.FUNI_ID) AS CORRECAO
...
if( $qry == "funaces" )
	...
	$sql	=	"SELECT		FUNI.FUNI_ID, FDTR.FDTR_ID, FDTR.TSDT_ID, 
										FUNI.PMS_IDPMSPESSOA as SSHD, PESS.DLNOMEPESSOA,
										to_char( FDTR.FDTR_DTREFERENCIA, 'DD/MM/YYYY' ) as Data,
                        ...
                        WHERE FDTN.FDTR_ID = FDTR.FDTR_ID) AS TIPOMENSAGEM, 
										...
										CASE FUCO_DCDBCR 
														WHEN 'DB' THEN 
															'Débito de '||BIOMETRIA.SF_TOHHMM( FUCO_NITMP)||' ' || FUCO.FUCO_DLOBS
														WHEN 'CR' THEN 
															'Crédito de '||BIOMETRIA.SF_TOHHMM( FUCO_NITMP)||' ' || FUCO.FUCO_DLOBS
														END  AS CORRECAO,
...
$jsres	= json_decode(str_replace( "\0", "", $res));
if( $dbg )
	{
