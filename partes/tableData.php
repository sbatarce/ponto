...
if( $qry == "funcindex" )
	...
	$autid = $_GET["autid"];
	$sql	=	"SELECT FUNI.FUNI_ID AS IDFUNC, VUPU.UOR_DLSIGLAUNIDADE AS UNIDADE, 
      (SELECT VPSS.NOME
  ...
  FROM BIOMETRIA.FUAU_FUNCIONARIOAUTORIZADOR FUAU
       INNER JOIN 
          (SELECT FUAUI.PMS_IDSAUUOR, UOFMI.UOFM_ID, UOFMI.UOFM_DTREFERENCIA
  ...
  WHERE SYSDATE BETWEEN FUAU.FUAU_DTINICIO AND NVL(FUAU.FUAU_DTFIM, 
      TO_DATE('31/12/3000', 'DD/MM/YYYY')) AND FUAU.FUNI_ID = $autid AND
      UOFM.UOFM_ID = 
      (SELECT UOFMF.UOFM_ID
          FROM (SELECT UOFMI.UOFM_ID, FUAUI.PMS_IDSAUUOR, UOFMI.UOFM_DTREFERENCIA
                  FROM BIOMETRIA.UOFM_UORFECHAMENTOMENSAL UOFMI
                  INNER JOIN BIOMETRIA.FUAU_FUNCIONARIOAUTORIZADOR FUAUI ON
                             FUAUI.FUAU_ID = UOFMI.FUAU_ID
                  ORDER BY UOFMI.UOFM_DTREFERENCIA DESC) UOFMF
          WHERE UOFMF.PMS_IDSAUUOR = FUAU.PMS_IDSAUUOR AND ROWNUM = 1)
  GROUP BY FUNI.FUNI_ID, FUNI.PMS_IDPMSPESSOA, 
           VUPU.UOR_DLSIGLAUNIDADE, UOFM.UOFM_DTREFERENCIA
