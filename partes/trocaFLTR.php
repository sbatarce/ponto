<?php
/**
 * altera o LOTR_ID no FLTR dados o FLTR_ID e o APAL_ID
 */
if( !isset( $_GET["fltrid"] ) )
	{
	echo	'{ "data": [{"erro": "parametro fltrid obrigatório"}] }';
	return;
	}
$fltrid = $_GET["fltrid"];
...
//	insere o novo 
$sql = "UPDATE BIOMETRIA.FLTR_FUNCIONARIOLOCALTRABALHO SET
					LOTR_ID=$idlotr WHERE FLTR_ID=$fltrid";
$res = $ora->execDelUpd( $sql );
$jres = json_decode( $res );
if( $dbg )
	{
	echo "update FLTR SQL=$sql/resultado:";
	var_dump($jres);
	}
if( $jres->status != "OK" )
	{
	fmtErro( "erro", "alterando FLTR: $jres->erro" );
	$ora->rollback();
	$ora->disconnect();
	return;
	}	
